...
\subsubsection{Testing environment and results}

The testing for this component was done running the GNS3 server in virtual machines on Informatics Department's servers.
These are machines with two Intel Xeon E5-2650 v4 @ 2.20GHz (a total of 24 cores) and 64 GB of RAM.
The virtual machine used to test the switching lab is running on the VMware ESXi bare metal hypervisor, with nested virtualization enabled (needed so appliances, which are themselves VMs, can be emulated), had 4 CPU cores, and 8 GB of RAM allocated.
